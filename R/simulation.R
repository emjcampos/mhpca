...
MHPCA_simulation <- function(
  ...
  eta_vec <- map(1:D, function(d) {
    map(1:J, function(j) { 
      map(reg, ~ 2 * (-1) ^ (j) * (func - (d - 1)) ^ 3)
    }) 
  })
  ...
  data <- expand.grid(
    ...
    pmap_dfr(function(d, j, i, r) { 
      ...
      data <- data.frame(
        ...
        reg = paste0(
          "E", 
          rep(str_pad(r, width = 2, pad = "0"), length(func))
        ),
        func = func,
        y_nonoise = subj_trajectory + level_1 + level_2,
        noise = rnorm(length(func), 0, sig_eps), 
        stringsAsFactors = FALSE
      ) %>% 
        mutate(y_noise = y_nonoise + noise)
      
      SNR <- var(data$y_nonoise) / sig_eps^2 
